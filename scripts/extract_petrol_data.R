...
# gridlines fixes the SPACING; OCR is used to pin both the
# cents-per-gridline STEP and the absolute LEVEL, and only
# plausible reads are allowed to vote.
calibrate_axis <- function(axis_map,
  ...
  # any of it is trusted. "multiple of 5" is about label
  # GRANULARITY and is independent of the (unknown) gridline
  # spacing, so it is safe to apply up front.
  ocr <- axis_map$axis_value
  ok  <- !is.na(ocr) &
    ocr >= plausible[1] & ocr <= plausible[2] &
    (ocr %% label_multiple == 0)
  
  if (sum(ok) < 2)
    stop("Need >= 2 plausible y-axis labels to infer the gridline step — ",
         "aborting instead of assuming one.")
  
  s <- slot[ok]; v <- ocr[ok]
  
  # Infer cents-per-gridline from the data (Theil-Sen slope).
  # value falls as image_row (slot) rises, so slope is negative.
  # The median of all pairwise slopes ignores up to ~29% bad
  # reads, so a stray misread can't set the step.
  pairs     <- combn(length(s), 2)
  dv        <- v[pairs[1, ]] - v[pairs[2, ]]
  ds        <- s[pairs[1, ]] - s[pairs[2, ]]
  keep      <- ds != 0
  raw_slope <- median(dv[keep] / ds[keep])
  
  # Snap |slope| to the nearest sensible axis step; refuse if it
  # is nowhere near one (that signals unreliable reads/slots —
  # better to flag for review than to fabricate an axis).
  step <- allowed_steps[which.min(abs(allowed_steps - abs(raw_slope)))]
  if (min(abs(allowed_steps - abs(raw_slope))) > 0.35 * step)
    stop(sprintf(paste0("Inferred gridline step (%.2f c/gridline) matches no ",
                        "expected step {%s}; aborting rather than guessing."),
                 abs(raw_slope), paste(allowed_steps, collapse = ", ")))
  
  # Each good read implies a value for the TOP gridline. Vote.
  v_top_votes <- round(v + step * s)
  tbl   <- sort(table(v_top_votes), decreasing = TRUE)
  v_top <- as.numeric(names(tbl)[1])
  agree <- as.integer(tbl[1])
  
  cat(sprintf("  Axis calibration: step = %g c/gridline (raw %.2f), top = %g  (%d/%d reads agree)\n",
              step, abs(raw_slope), v_top, agree, sum(ok)))
  if (agree < 2)
    cat("  WARNING: weak axis level consensus (only one supporting read) — ",
        "verify the check plot.\n")
  
  # Consistency check: do the good reads actually sit on this axis?
  fitted <- v_top - step * s
  bad    <- which(abs(v - fitted) > step / 2)
  if (length(bad) > 0)
    cat(sprintf("  NOTE: %d OCR read(s) disagreed with the fitted axis and were overridden.\n",
                length(bad)))
  
  # Rebuild a perfectly even axis from the agreed step + level.
  axis_map$axis_value <- v_top - step * slot
  axis_map
}
...
extract_chart_data <- function(image_path, city_name, target_dots = 45,
  ...
  axis_map <- calibrate_axis(axis_map,
                             allowed_steps  = allowed_steps,
                             label_multiple = label_multiple,
                             plausible      = plausible)
